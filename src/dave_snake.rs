...
impl Point {
	...
	fn transform(&self, direction: Direction, times: u16) -> Self {
	...
	fn transform_value(value: u16, by: i16) -> u16 {
		if by.is_negative() && by.abs() as u16 > value {
			panic!("Transforming value {} by {} would result in a negative value", value, by);
		} else {
...
impl Snake {
	fn new(start: Point, length: u16, direction: Direction) -> Self {
			...
			let symbol = if let Some(&next) = next {
				if let Some(&previous) = previous {
					if previous.x == next.x {
						...
						let down = body.transform(Direction::Down, 1);
						let right = body.transform(Direction::Right, 1);
						let up = if body.y == 0 { body.clone() } else { body.transform(Direction::Up, 1) };
						let left = if body.x == 0 { body.clone() } else { body.transform(Direction::Left, 1) };
						if (next == down && previous == right) || (previous == down && next == right) {
							'╔'
						} else if (next == down && previous == left) || (previous == down && next == left) {
							'╗'
						} else if (next == up && previous == right) || (previous == up && next == right) {
							'╚'
						} else {
							'╝'
						}
